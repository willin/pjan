/*!
 * wpjan(ajax + history.pushState + json) for jquery
 * Version: 1.1
 * Author: Willin Wang
 * URI: http://willin.org
 */
(function($) {
    var Util = {
        ...
        getRealUrl : function(url) {
            url = (url || '').replace(/\#.*?$/, '');
            url = url.replace('?pjan=true&', '?').replace('?pjan=true', '').replace('&pjan=true', '');
            return url;
        },
        // 获取url的hash部分
    ...
    pjan.showFx = {
        ...
        direct : function(data, callback, isCached) {
            this.html(data);
            callback && callback.call(this, data, isCached);
        },
